...
exports.start = (app, module) =>
{
  ...
  module.send = function(to, subject, text, done)
  {
    ...
    if (module.config.replyTo && module.config.overrideReplyTo)
    {
      mailOptions.replyTo = module.config.replyTo;
    }

    if (done)
    {
      return send(mailOptions, done);
    }
  ...
  function send(mailOptions, done)
  {
    if (module.config.remoteSenderUrl !== null)
    {
      return sendThroughRemote(mailOptions, done);
    }

    if (transport !== null)
    {
      return sendThroughSmtp(mailOptions, done);
    }

    if (module.config.emailsPath !== null)
    {
      return sendThroughFile(mailOptions, done);
    }
  ...
  function sendThroughSmtp(mailOptions, done)
  {
    transport.sendMail(mailOptions, err =>
    {
      ...
      else
      {
        ...
        cleanUpAttachments(mailOptions.attachments);
      }
  ...
  function sendThroughFile(mailOptions, done)
  {
    ...
    _.forEach(mailOptions.attachments, attachment =>
    {
      if (!attachment.path)
      {
        return;
  ...
  function sendFromFile(fileInfo)
  {
    ...
    fs.readFile(fileInfo.filePath, 'utf8', (err, contents) =>
    {
      ...
      _.forEach(headers, header =>
      {
        ...
        if (headerName === 'attachments')
        {
          const remove = headerValue.startsWith('-');
          ...
          mailOptions.attachments.push({
            path: remove ? headerValue.substring(1) : headerValue,
            remove
          });
  ...
  function cleanUpAttachments(attachments)
  {
    ...
    {
      if (!attachment.path || attachment.remove !== true)
      {
        return;
      }
      ...
      fs.unlink(attachment.path, err =>
      {
        if (err)
        {
          module.warn(err, 'Failed to remove attachment.', {attachment});
        }
      });
    });
  }
