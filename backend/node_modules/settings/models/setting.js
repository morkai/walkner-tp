...
exports.setUp = (app, mongoose) =>
{
  ...
  schema.statics.findValue = function(id, done)
  {
    if (!done)
    {
      return this.findById(id).lean().exec().then(setting => setting ? setting.value : null);
    }

    this.findById(id).lean().exec().then(
      setting => done(null, setting ? setting.value : null),
      done
    );
  };
  ...
  schema.statics.findValues = function(conditions, ns, done)
  {
    ...
    function prepareValues(settings)
    {
      ...
      {
        values[ns ? setting._id.replace(ns, '') : setting._id] = setting.value;
      });
