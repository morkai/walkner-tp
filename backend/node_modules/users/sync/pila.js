...
module.exports = async (app, module, options) =>
{
  ...
  const fromRemoteDb = !Array.isArray(options.users);
  const fromLocalDb = !fromRemoteDb && options.users.length === 0;
  let tedious = null;
  ...
  return new Promise((resolve, reject) =>
  {
    if (fromRemoteDb)
    {
      ...
      {
        return reject(app.createError('No tedious module.', 'DEPENDENCIES'));
      }

      if (!options.tediousConnection)
      {
        return reject(app.createError('No tedious config.', 'DEPENDENCIES'));
      }
    }
    ...
    step(
      function()
      {
        ...
        cursor.on('data', user =>
        {
          if (fromLocalDb)
          {
            if (user.syncData.props)
            {
              user.syncData.props = _.without(user.syncData.props, ...SYNCED_ORG_UNIT_PROPS);
            }
            else
            {
              user.syncData.props = [...SYNCED_PROPS];
            }
      ...
      function(err, matchers)
      {
        ...
        prepareNewUsers(this.conn, matchers, options.users || [], this.next());
      },
      function(err, newUsers)
      {
        ...
        syncNextUser(stats, newUsers, 0, this.missingUsers, new Set(), this.next());
      },
      function(err, stats)
      {
        if (err)
  ...
  function prepareNewUsers(conn, matchers, newUsers, done)
  {
    if (newUsers.length)
    {
      const users = [];
      ...
      {
        const filter = filterUser(newUser);
        ...
        if (!newUser.syncData.props)
        {
          newUser.syncData.props = [...SYNCED_PROPS];
        }
    ...
    if (!conn)
    {
      return done(app.createError('No tedious connection.', 'INPUT', 400));
    }
  ...
  function filterUser(newUser)
  {
    if (!newUser.syncData || typeof newUser.syncData.company !== 'string')
    {
    ...
    if (newUser.syncData.company.includes('Signify'))
    {
      if (/Pi.a/.test(newUser.syncData.company))
      {
        return 'assign';
      }
    ...
    if (newUser.syncData.workplace === 'ZEWN')
    {
      return 'keep';
    }
  ...
  function applyMatcher(matchers, input, newUser)
  {
    ...
    if (matcher)
    {
      ...
      newUser.syncData.props.push(...SYNCED_ORG_UNIT_PROPS);
    }
  }

  function syncNextUser(stats, newUsers, newUserI, missingUsers, activeSyncIds, done)
  {
    ...
    step(
      ...
      {
        if (!newUser.syncId || activeSyncIds.has(newUser.syncId))
        {
          return this.skip();
        }
      ...
      function prepareUserModelStep(err, users)
      {
        ...
        if (activeUser)
        {
          if (jsonDeepEqual(newUser.syncData, activeUser.syncData))
          {
            delete newUser.syncData;
          }
        ...
        activeSyncIds.add(activeUser.syncId);
