...
module.exports = async (app, module, options) =>
{
  ...
  return new Promise((resolve, reject) =>
  {
    ...
    if (!options || !options.tediousConnection)
    {
  ...
  function createSelectUsersSql()
  {
    return `
      ...
      FROM USERS
      WHERE US_TYPE <> 2
        AND US_PERSONELLID IS NOT NULL
        AND US_CARDS_NUMBER_ACTIVE_STR IS NOT NULL
        AND US_GROUP_NAME <> 'Zwolnieni'
      ORDER BY US_ID ASC
    `;
  }
  ...
  function prepareNewUsers(conn, done)
  {
    ...
    req.on('row', row =>
    {
      ...
      const newUser = {
        ...
        syncId: personnelId,
        syncData: {
          props: ['personnelId', 'lastName', 'firstName', 'card', 'company', 'gender'],
          company
        }
      };
