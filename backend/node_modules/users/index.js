...
exports.republishTopics = [
  'users.synced', 'users.syncFailed',
  'users.presence.updated'
];

exports.userPrivileges = ['SUPER', 'USERS:VIEW', 'USERS:MANAGE'];

exports.optionalModules = {
  'mongoose': require('./deactivator'),
...
exports.start = (app, module) =>
{
  ...
  {
    if (module.config.syncOptions && module.config.syncOptions.auto === false)
    {
      return;
    }
  ...
  async function syncUsers(options = {}, done)
  {
    if (!done)
    {
      done = () => {};
    ...
    {
      const t = Date.now();
      const stats = await syncHandler(app, module, {..._.cloneDeep(syncOptions), ...options});
    ...
    catch (err)
    {
      ...
      app.broker.publish('users.syncFailed', {user: options.user, error: err.message});
    ...
    if (!options.user)
    {
      setTimeout(scheduleNextUserSync, 120 * 1000);
    }
  }
};
