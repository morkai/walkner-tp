...
exports.setUp = (app, mongoose) =>
{
  ...
  {
    input = _.pick(input, USER_PROPERTIES);
  ...
  transportOrderSchema.methods.applyChanges = function(input, updater, roles, createUserInfo)
  {
    ...
    var comment = _.isString(input.comment) ? input.comment.trim() : '';
    var changes = this.compareProperties(_.pick(input, inputProperties), createUserInfo);
  ...
  transportOrderSchema.methods.populateUserData = function(done)
  {
    ...
    return populate ? this.populate().then(() => done(null, this), done) : done(null, this);
  };
  ...
  transportOrderSchema.methods.compareProperties = function(input, createUserInfo)
  {
    ...
    if (_.isString(input.status))
    {
      properties = _.without(properties, 'status');
  ...
  transportOrderSchema.methods.updateLastSeenProperties = function(changedProperties, changes, updaterId)
  {
    ...
    else
    {
      ...
      this.creatorChanges = _.uniq([].concat(this.creatorChanges || [], changedProperties));
    }
    ...
    else
    {
      ...
      this.ownerChanges = changes.owner ? null : _.uniq([].concat(this.ownerChanges || [], changedProperties));
    }
    ...
    else
    {
      ...
      this.dispatcherChanges = changes.dispatcher
        ? null
        : _.uniq([].concat(this.dispatcherChanges || [], changedProperties));
    }
    ...
    else
    {
      ...
      this.driverChanges = changes.driver ? null : _.uniq([].concat(this.driverChanges || [], changedProperties));
    }
  };
  ...
  mongoose.model(exports.name, transportOrderSchema);
};
